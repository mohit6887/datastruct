...
int max(int a, int b);
int getmaxdepth(struct node *p);
int diameterofbt(struct node *p);
int findiameter(struct node *p,int x);
int main()
{
	printf(" inside the %s \n",__func__);
	...
	while(1)
	{
		...
		switch(choice)
		{
			...
			case 12:
				printf(" maximum diameter of the tree \n");
				x=diameterofbt(root);
				printf("Maximum diameter = %d\n",x);
...
int getmaxdepth(struct node* p)
{
	...
	return 1+max(getmaxdepth(p->lchild),getmaxdepth(p->rchild));
...
{
	int diameter;
	diameter=findiameter(p,diameter);
	 return diameter;

}


int findiameter(struct node *p,int diameter)
{
	if(p== NULL)
	{
		return 0;
	}
	 int lh=findiameter(p->lchild,diameter);
	 int rh=findiameter(p->rchild,diameter);
